...
file1 = open(logs_dir + "LSTMdist+class_accuracies.txt","w")
for epoch in range(1, epochs+1):
    ...
    for steps, (true_gauss_tensor, true_wind_tensor, labels) in tqdm(enumerate(train_dataloader),total=len(train_dataloader)):        
        ...
        train_feature_vector.append(genuine_output.detach().cpu())
        loss.backward()
        optimizer.step()
    train_embedding_vector = torch.cat(train_feature_vector,dim=0)
    ...
    with torch.no_grad():
        for steps, (true_gauss_tensor, true_wind_tensor, labels) in tqdm(enumerate(val_dataloader),total=len(val_dataloader)):
            ...
            val_feature_vector.append(genuine_output.detach().cpu())
    
        val_embedding_vector = torch.cat(val_feature_vector,dim=0)


    # Forward pass
    train_embedding_vector = train_embedding_vector.numpy()
    val_embedding_vector = val_embedding_vector.numpy()
    _, y_targets = one_hot_labels.clone().max(dim=1)
    identify_model.fit(train_embedding_vector, y_targets.numpy()[0:train_data_len])
    outputs = identify_model.predict(val_embedding_vector)
    outputs = torch.from_numpy(outputs)
    # one-hot表現に変換
    predicted_labels = torch.zeros(outputs.size(0), 2)
    predicted_labels.scatter_(1, outputs.cpu().unsqueeze(1), 1)
    correct = (predicted_labels.to(device1) == one_hot_labels[train_data_len:val_data_len].to(device1)).sum().item()
    total = one_hot_labels[train_data_len:val_data_len].numel()
    accuracy = correct / total

    now_time = dt.datetime.now()
    print(f"EPOCH {epoch}| Train: loss {np.mean(train_steps_losses)} | Val: dist loss {np.mean(val_steps_losses)} | train accuracy {accuracy} ")
    file1.write("%s , %s, %s, %s, %s, %s\n" % (str(epoch), "train_loss", str(np.mean(train_steps_losses)), "train_accuracy", str(accuracy), now_time))
